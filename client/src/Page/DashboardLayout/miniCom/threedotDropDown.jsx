...
export default function MiniDropDown({postInfo,toggle}) {
    ...
    return(
        <div className="miniDropHome h-auto w-55 flex bg-skin-bg/20 backdrop-blur-lg items-center justify-center flex-wrap absolute top-full right-0">
            <ul>
                <li className="text-red-500 font-bold"><i className="bx bx-error"></i> Report</li>
                ...
                <li data-setting={"likeCount"} onClick={(evnt)=>handleClick(evnt.target.dataset.setting,postInfo.post_id)} ><i name="likeCount" className="bx bxs-star"></i>{(postInfo.likeCount === 1) ? "Hide Star Count":"Show start Count"}</li>
