...
export const acceptSolution = async (rkv,rspo) => {
    ...
    try {
        ...
        let [rows] = await database.query("SELECT p.id AS owner_id, p.post_moment, c.id AS commentOwner_id, c.isAccepted FROM comments c INNER JOIN posts p ON c.post_id = p.post_id WHERE c.commentID = ?",
            [commentID]
        );
        if (rows.length === 0) return rspo.status(404).send({err:"The comment is Deleted or hidden"});
        let {owner_id,post_moment,commentOwner_id,isAccepted} = rows[0];
